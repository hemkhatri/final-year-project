    ...
    try:
        ...
        if status == 'COMPLETE':
            ...
            if shop_order.status == 'PENDING':
                ...
                shop_order.transaction_uuid = f"{shop_order.transaction_uuid}-DELIVERY-{delivery_order.id}"
                ...
                try:
                    ...
                    print(f"🚚 Delivery Order #{delivery_order.id} - Assigned: {assigned}")
                except Exception as assign_error:
                    # Capture assignment layout issues (like missing URL names) safely 
                    print(f"⚠️ Payment succeeded, but notification assignment crashed: {str(assign_error)}")
                
                print(f"✅ Payment Success - Order #{order_id}")
                print(f"📍 Delivery Coords: {shop_order.delivery_latitude}, {shop_order.delivery_longitude}")
                ...
                return render(request, 'payment/success.html', {'shop_order': shop_order})
                
        else:
            return HttpResponse("eSewa payment transaction was not complete.", status=400)
            
    except Exception as e:
        print(f"❌ Payment Error: {str(e)}")
        return HttpResponse(f"Error parsing checkout payment success: {str(e)}", status=400)
